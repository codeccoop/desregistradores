...
function Form() {
    ...
    function onSubmitError(){
        setMessage({
            content: "Hi ha hagut un problema amb el formulari i no s'ha desat. Si us plau, intenta-ho de nou.",
            type: "danger"
        });
        formReset();
    }
        ...
        if (message.type != null) {
            return (
                <div className={"notification is-" + message.type}>
                    <button className="delete" onClick={()=>setMessage({content: null, type: null})}></button>
                    {message.content}
                </div>
            );
        } else {
